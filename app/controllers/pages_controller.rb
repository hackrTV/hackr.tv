class PagesController < ApplicationController
  skip_forgery_protection only: :not_found

  def spa_root
    # SPA root - renders minimal shell for React
    render layout: "application"
  end

  def hackr_tv
    # hackr.tv home page
  end

  def thecyberpulse
    render "mobile/thecyberpulse" if mobile?
  end

  def xeraen
    render "mobile/xeraen" if mobile?
  end

  def system_rot
    @artist = Artist.find_by(slug: "system-rot")
    @tracks = @artist.tracks.includes(:release).release_order if @artist
  end

  def wavelength_zero
    @artist = Artist.find_by(slug: "wavelength-zero")
    @tracks = @artist.tracks.includes(:release).release_order if @artist
  end

  def voiceprint
    @artist = Artist.find_by(slug: "voiceprint")
    @tracks = @artist.tracks.includes(:release).release_order if @artist
  end

  def temporal_blue_drift
    @artist = Artist.find_by(slug: "temporal-blue-drift")
    @tracks = @artist.tracks.includes(:release).release_order if @artist
  end

  def sector_x
  end

  def not_found
    render file: Rails.public_path.join("404.html"), layout: false, status: :not_found
  end
end
